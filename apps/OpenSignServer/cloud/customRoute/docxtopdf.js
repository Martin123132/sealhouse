...
import { cloudServerUrl, getSecureUrl, serverAppId, supportEmail } from '../../Utils.js';
...
export default async function docxtopdf(req, res) {
  ...
  try {
    ...
    const supportText = supportEmail ? ` or contact ${supportEmail}` : '';
    const message =
      `We are currently experiencing some issues with processing DOCX files. Please upload the PDF version${supportText}.`;

    if (msg.includes('timed out')) {
      msg =
        `Document conversion is taking too long. Please try a smaller file${supportText}.`;
    } else if (msg.includes('too large') || msg.includes('size')) {
      msg =
        `File is too large to process. Please reduce the file size${supportText}.`;
    } else {
      msg = message;
    }
