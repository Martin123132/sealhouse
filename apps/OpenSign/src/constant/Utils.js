...
export const embedWidgetsToDoc = async (
  ...
  if (!hasError) {
    ...
    return {
      error:
        "This PDF is not compatible with this signing instance."
    };
  }
};
...
export const getAppLogo = async () => {
  ...
  try {
    ...
    if (tenant) {
      ...
      localStorage.setItem("appname", "Sealhouse");
...
export const mailTemplate = (param) => {
  const appName = "Sealhouse";
  const logo = process.env.REACT_APP_MAIL_LOGO_URL
    ? `<div style='padding:10px'><img src='${process.env.REACT_APP_MAIL_LOGO_URL}' height='50' /></div>`
    : "";
